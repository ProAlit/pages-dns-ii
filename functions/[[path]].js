/**
 * VERSION: 3.1.0 (5-Way Shotgun Racing Engine)
 * Runtime: Cloudflare Pages (_worker.js) / Workers
 */
...
const CONFIG = {
  ...
  RACE_COUNT: 5, // 🏁 Racing 5 simultaneous connections
  SCORE_START: 100,
  SCORE_MIN: 0,
  SCORE_MAX: 100,
...
// Single ControlD Upstream duplicated 5 times for the racing engine
const CONTROL_D_URL = 'https://freedns.controld.com/no-ads-dating-drugs-gambling-typo-malware';
const UPSTREAM_DOH = [
  CONTROL_D_URL,
  CONTROL_D_URL,
  CONTROL_D_URL,
...
function buildResolverState(profiles) {
  ...
  for (const [profile, urls] of Object.entries(profiles)) {
    state[profile] = urls.map((url) => ({
      url, score: CONFIG.SCORE_START, ok: 0, fail: 0, timeout: 0, lastLatencyMs: null, lastError: null
    }));
  }
  return state;
}
...
export default {
  async fetch(req, env, ctx) {
    ...
    if (url.pathname === '/health') {
      return jsonResponse(getHealthSnapshot(), 200, { 'cache-control': 'no-store' });
    }

    // 2. Rate Limiting (Disguised as 404)
    if (checkSpam(clientIP)) {
      return textResponse('Not Found', 404, { 'cache-control': 'no-store' }); 
    }

    // 3. ALL paths (including /) are processed as DNS. 
...
async function handleDNS(req, url) {
  ...
  try {
    ...
    // If it's a normal browser GET or an invalid POST, return 404 Not Found
    return textResponse('Not Found', 404, { 'cache-control': 'no-store' });
  }

  if (!payload || payload.byteLength === 0) return textResponse('Not Found', 404, { 'cache-control': 'no-store' });
  if (payload.byteLength > CONFIG.MAX_DNS_MESSAGE_BYTES) return textResponse('Not Found', 404, { 'cache-control': 'no-store' });

  const parsed = parseDNSQuestion(payload);
  if (!parsed.ok) return textResponse('Not Found', 404, { 'cache-control': 'no-store' });
  ...
  if (hit) {
    ...
    return dnsResponse(responseBody, { 'x-cache': 'HIT', 'x-profile': profile });
  }

  const racers = selectRacers(resolvers);
  try {
    const winner = await raceResolvers(racers, payload, parsed.id);
    if (isCacheableDNSResponse(winner.body)) setCache(cacheKey, normalizeDNSResponseID(winner.body), CONFIG.CACHE_TTL_SECONDS);
    
    return dnsResponse(winner.body, {
      'x-cache': 'MISS', 'x-profile': profile,
      'x-winner': sanitizeHeaderValue(winner.url), 'x-winner-lat': `${winner.latencyMs}ms`
    });
  } catch (err) {
    return textResponse('Not Found', 404, { 'cache-control': 'no-store', 'x-profile': profile });
  }
}
...
async function readDNSPayload(req, url) {
  if (req.method === 'GET') {
    ...
    if (!q) throw httpError('Not a DNS request', 404);
    return decodeBase64Url(q);
  }
  const contentType = req.headers.get('content-type') || '';
  if (!contentType.toLowerCase().includes('application/dns-message')) throw httpError('Not a DNS request', 404);
  return new Uint8Array(await req.arrayBuffer());
}

function decodeBase64Url(input) {
  if (!/^[A-Za-z0-9_-]+$/.test(input)) throw httpError('Invalid base64url DNS query', 400);
  let normalized = input.replace(/-/g, '+').replace(/_/g, '/');
  normalized += '='.repeat((4 - (normalized.length % 4)) % 4);
  try { return Uint8Array.from(atob(normalized), (c) => c.charCodeAt(0)); } 
  catch (_) { throw httpError('Invalid base64url DNS query', 400); }
}

function parseDNSQuestion(packet) {
  const bytes = packet instanceof Uint8Array ? packet : new Uint8Array(packet);
  if (bytes.byteLength < 12) return { ok: false, error: 'DNS message too short' };
  const id = (bytes[0] << 8) | bytes[1];
  const flags = (bytes[2] << 8) | bytes[3];
  const qdcount = (bytes[4] << 8) | bytes[5];
  if ((flags & 0x8000) !== 0) return { ok: false, error: 'DNS query expected, got response' };
  if (qdcount !== 1) return { ok: false, error: 'Exactly one DNS question is required' };
  ...
  while (offset < bytes.length) {
    ...
    if (len === 0) break;
    if ((len & 0xc0) !== 0) return { ok: false, error: 'Compressed question names are not accepted' };
    if (len > 63 || offset + len > bytes.length) return { ok: false, error: 'Invalid DNS question name' };
    let label = '';
    for (let i = 0; i < len; i++) label += String.fromCharCode(bytes[offset++]).toLowerCase();
    labels.push(label);
  }
  if (offset + 4 > bytes.length) return { ok: false, error: 'Incomplete DNS question' };
  const qtype = (bytes[offset] << 8) | bytes[offset + 1];
  const qclass = (bytes[offset + 2] << 8) | bytes[offset + 3];
  const qname = labels.join('.') || '.';
  return { ok: true, id, qname, qtype, qclass, questionKey: `${qname}|${qtype}|${qclass}` };
}
...
async function raceResolvers(nodes, packet, expectedID) {
  ...
  try {
    ...
    for (const controller of controllers) controller.abort('winner-selected');
    return winner;
  } finally {
    for (const controller of controllers) controller.abort('race-finished');
  }
}
...
async function relay(node, packet, expectedID, signal) {
  ...
  try {
    const res = await fetch(node.url, {
      ...
      body: packet, signal: combinedSignal
    });
    if (!res.ok) { penalize(node, CONFIG.SCORE_FAILURE_DELTA, `HTTP ${res.status}`); throw new Error(`Upstream HTTP ${res.status}`); }
    const body = await res.arrayBuffer();
    const validation = validateDNSResponse(body, expectedID);
    if (!validation.ok) { penalize(node, CONFIG.SCORE_FAILURE_DELTA, validation.error); throw new Error(validation.error); }
    const latencyMs = Date.now() - started;
    reward(node, latencyMs);
    return { url: node.url, body, latencyMs };
  } catch (err) {
    const message = String(err && err.message ? err.message : err);
    if (message.includes('abort') || message.includes('timeout') || timeoutController.signal.aborted) {
      node.timeout += 1; penalize(node, CONFIG.SCORE_TIMEOUT_DELTA, 'timeout');
    } else { penalize(node, CONFIG.SCORE_FAILURE_DELTA, message); }
    throw err;
...
function anySignal(signals) {
  ...
  function abortFrom(signal) { if (!controller.signal.aborted) controller.abort(signal.reason || 'aborted'); }
...
function reward(node, latencyMs) {
  node.ok += 1; node.lastLatencyMs = latencyMs; node.lastError = null;
...
function penalize(node, amount, error) {
  node.fail += 1; node.lastError = String(error || 'unknown').slice(0, 80);
...
function getCache(key) {
  ...
  if (!item) return null;
  if (Date.now() > item.expiresAt) { APP_STATE.cache.delete(key); return null; }
  APP_STATE.cache.delete(key); APP_STATE.cache.set(key, item);
  return item;
}

function setCache(key, body, ttlSeconds) {
  APP_STATE.cache.set(key, { body, expiresAt: Date.now() + ttlSeconds * 1000 });
...
function checkSpam(ip) {
  ...
  if (now > stats.resetAt) stats = { count: 0, resetAt: now + CONFIG.RATE_LIMIT_WINDOW_MS };
  stats.count += 1; APP_STATE.throttle.set(ip, stats);
...
function trimMap(map, maxEntries) {
  while (map.size > maxEntries) { const oldestKey = map.keys().next().value; map.delete(oldestKey); }
}
...
function getHealthSnapshot() {
  ...
  for (const [profile, nodes] of Object.entries(APP_STATE.resolversByProfile)) {
    profiles[profile] = nodes.map((node, index) => ({
      // Added index to health snapshot so you can tell the 5 identical nodes apart
      node_index: index, 
      url: node.url, score: node.score, ok: node.ok, fail: node.fail, timeout: node.timeout,
      lastLatencyMs: node.lastLatencyMs, lastError: node.lastError
    }));
  }
  return { version: VERSION, cacheEntries: APP_STATE.cache.size, throttleEntries: APP_STATE.throttle.size, profiles };
}

function dnsResponse(body, extraHeaders = {}) {
  return new Response(body, { status: 200, headers: { 'content-type': 'application/dns-message', 'cache-control': 'no-store', ...extraHeaders } });
}
...
function sanitizeHeaderValue(value) { return String(value).replace(/[\r\n]/g, '').slice(0, 200); }
